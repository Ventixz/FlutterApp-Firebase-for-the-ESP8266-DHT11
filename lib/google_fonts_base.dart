...
import 'package:google_fonts/src/file_io.dart' // Stubbed implementation by default.
// Concrete implementation if File IO is available.
if (dart.library.io) 'package:google_fonts/src/file_io_desktop_and_mobile.dart' as file_io;
...
@visibleForTesting
google_fonts_asset_manifest.AssetManifest assetManifest = google_fonts_asset_manifest.AssetManifest();
...
Future<void> loadFontIfNecessary(GoogleFontsDescriptor descriptor) async {
  ...
  try {
    ...
    final assetManifestJson = await google_fonts_asset_manifest.AssetManifest().json();
    final assetPath = _findFamilyWithVariantAssetPath(
      descriptor.familyWithVariant,
      assetManifestJson,
    );
    if (assetPath != null) {
      byteData = flutter_services.rootBundle.load(assetPath);
    }
    ...
    if (GoogleFonts.config.allowRuntimeFetching) {
      byteData = _httpFetchFontAndSaveToDevice(fontName, descriptor.file);
      if (await byteData != null) {
        return loadFontByteData(familyWithVariantString, byteData);
      }
    } else {
      throw Exception('Runtime fetching is not allowed.');
    }
  } catch (e) {
